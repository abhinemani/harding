...
export default function ProgramLayerBuilder() {
  ...
  function buildHtml() {
    ...
    const index = `<table class="glance"><thead><tr><th>Program</th><th>Serves</th><th>Delivers</th><th>So that</th><th class="r">Cost</th><th class="r">FTE</th><th>Mandate</th><th>Alignment</th></tr></thead><tbody>${sorted.map((p) => { const st = parseStatement(p.statement); const none = p.alignment === "none" || /none identified/i.test(p.strategic_linkage || "");
      return `<tr><td><div class="pn">${esc(p.name)}</div><div class="sub" style="color:${p.facing === "internal" ? "#2E7D6E" : C.maroon}">${p.facing === "internal" ? "internal · performance" : "external · outcomes"}</div></td><td>${esc(p.beneficiary || st.beneficiary || "—")}</td><td>${esc(st.service || "—")}</td><td>${esc(st.result || "—")}</td><td class="r">${money(p.cost)}<div class="bar" style="width:72px;margin:4px 0 0 auto"><span style="width:${(p.share / (sorted[0].share || 1)) * 100}%;background:${p.is_admin ? C.grey : C.maroon}"></span></div><div class="sub">${Math.round(p.share * 100)}%</div></td><td class="r">${Number(p.fte || 0).toFixed(2)}</td><td style="color:${p.mandate?.status === "discretionary" ? C.gold : C.ink}">${esc(p.mandate?.status || "—")}${p.mandate?.citation ? `<div class="sub">${esc(p.mandate.citation)}</div>` : ""}</td><td style="color:${none ? C.grey : p.alignment === "direct" ? C.gold : C.ink}">${none ? "not aligned" : esc(p.alignment || "—")}${!none && p.strategic_linkage ? `<div class="sub">${esc(p.strategic_linkage)}</div>` : ""}</td></tr>`; }).join("")}</tbody></table>`;
                      ...
                      <tbody>
                        {sorted.map((p) => { const st = parseStatement(p.statement); const none = p.alignment === "none" || /none identified/i.test(p.strategic_linkage || "");
                          ...
                          <tr key={p.name} style={{ borderBottom: `1px solid ${C.line}`, verticalAlign: "top" }}>
                            ...
                            <td style={{ padding: "8px 8px 8px 4px", minWidth: 140, color: C.ink }}>{p.beneficiary || st.beneficiary || "—"}</td>
                            <td style={{ padding: "8px 8px 8px 4px", minWidth: 150 }}>{st.service || "—"}</td>
                            <td style={{ padding: "8px 8px 8px 4px", minWidth: 170 }}>{st.result || "—"}</td>
                            ...
                            <td style={{ padding: "8px 8px 8px 4px", minWidth: 110, color: p.mandate?.status === "discretionary" ? C.gold : C.ink }}>{p.mandate?.status || "—"}{p.mandate?.citation && <div style={{ fontSize: 10.5, color: C.grey }}>{p.mandate.citation}</div>}</td>
                            <td style={{ padding: "8px 4px 8px 4px", minWidth: 120, color: none ? C.grey : p.alignment === "direct" ? C.gold : C.ink }}>{none ? "not aligned" : p.alignment || "—"}{!none && p.strategic_linkage && <div style={{ fontSize: 10.5, color: C.grey }}>{p.strategic_linkage}</div>}</td>
                          </tr>);
                        })}
                      </tbody>
                    </table>
                    ...
                    {sorted.map((p) => { const st = parseStatement(p.statement); const none = p.alignment === "none" || /none identified/i.test(p.strategic_linkage || "");
                      ...
                      <a key={p.name} href={`#prog-${encodeURIComponent(p.name)}`} style={{ color: C.ink, textDecoration: "none", borderTop: `1px solid ${C.line}`, paddingTop: 8 }}>
                        ...
                        <div style={{ fontFamily: sans, fontSize: 12, lineHeight: 1.5, color: C.grey }}>
                          <div><span style={{ color: C.ink }}>Serves</span> {p.beneficiary || st.beneficiary || "—"}</div>
                          {st.result && <div><span style={{ color: C.ink }}>So that</span> {st.result}</div>}
                          <div>{p.mandate?.status || "—"} · {none ? "not aligned" : p.alignment || "—"} · {p.facing === "internal" ? "internal" : "external"}</div>
                        </div>
                      </a>);
                    })}
                  </div>
