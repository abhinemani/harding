...
Division: ${division || "(unnamed)"}. Infer what it does from its name and the costing center and account names, using your knowledge of how Maryland county governments are organized. Decide whether it is external-facing (residents are the beneficiaries) or internal (other divisions or employees are). Internal divisions state a service level and the work it enables; do not force them into public-outcome language.
...
Measures. Exactly three per program, of three kinds: a RESULT measure (did the beneficiary's condition change), a VOLUME measure (how much was delivered), and a UNIT COST (cost ÷ volume, using the volume measure). Each must come from a system that already produces it (Munis, HRIS, applicant tracking, CRM, work orders, permit and inspection systems, carrier or actuary reports, state reports); if the number does not exist today, name the closest proxy and rate exists_today honestly. A deadline is not a measure.

Mandate status: one of "required by law" (cite it), "required to receive funding" (name the source), or "discretionary". Strategic linkage: the specific Livable Frederick element, adopted plan, or Executive priority, or "none identified" — an honest blank beats a stretch. The twenty percent question: what specifically stops at −20%, and what becomes possible at +20% — two concrete sentences, in specifics not generalities.
...
export default function ProgramLayerBuilder() {
  ...
  function buildHtml() {
    ...
    const cards = sorted.map((p, i) => {
      ...
      const none = /none identified/i.test(p.strategic_linkage || "");
      return `<section class="card${p.is_admin ? " admin" : ""}">
        <div class="head"><div><div class="lbl">Program ${i + 1} of ${sorted.length}${p.is_admin ? " · administration" : ""}</div><h2>${esc(p.name)}</h2>
          ...
          <div class="chips">${p.mandate?.status ? `<span class="chip ${p.mandate.status === "discretionary" ? "gold" : ""}">${esc(p.mandate.status)}${p.mandate.citation ? ` · ${esc(p.mandate.citation)}` : ""}</span>` : ""}${p.strategic_linkage ? `<span class="chip ${none ? "grey" : "maroon"}">${esc(p.strategic_linkage)}</span>` : ""}</div></div>
          <div class="cost"><div class="big">${money(p.cost)}</div><div class="lbl">${Math.round(p.share * 100)}% of this budget · ${Number(p.fte || 0).toFixed(2)} FTE</div>${strip}</div></div>
        ...
        <div class="measures three">${cell("Result", "did the condition change", m.result)}${cell("Volume", "how much was delivered", m.volume)}${cell("Unit cost", "cost ÷ volume", m.unit_cost)}</div>
        <div class="twenty"><div><span class="lbl" style="color:${C.maroon}">At −20%</span><div>${esc(p.twenty_percent?.minus || "—")}</div></div><div><span class="lbl" style="color:${C.gold}">At +20%</span><div>${esc(p.twenty_percent?.plus || "—")}</div></div></div>${lines}</section>`;
    }).join("\n");
...
<style>
  ...
  <div class="mast"><div class="kick">Line of Sight · program layer builder</div><h1>${esc(division || "Division")}: proposed program layer</h1>
    <p>Pre-decisional draft to the County Program Standard · ${today} · ${money(total)} across ${programs.length} programs and ${programs.reduce((a, p) => a + (Number(p.fte) || 0), 0).toFixed(1)} FTE · ${Math.round(coverage * 100)}% of budget mapped · ${esc(result.orientation)}-facing</p></div>
  ...
  ${index}
  <div class="legend"><span>Data readiness:</span><span>${dot("likely")}exists today</span><span>${dot("possible")}could be assembled</span><span>${dot("unlikely")}would need new collection</span></div>
  ...
  function buildMarkdown() {
    ...
    sorted.forEach((p, i) => {
      const m = p.measures || {};
      L.push("", `## ${i + 1}. ${p.name}`, "", `> ${p.statement}, at ${money(p.cost)} and ${Number(p.fte || 0).toFixed(2)} FTE in FY26.`, "");
      if (p.beneficiary) L.push(`**Beneficiary:** ${p.beneficiary}  `);
      L.push(`**Mandate:** ${p.mandate?.status || "—"}${p.mandate?.citation ? ` — ${p.mandate.citation}` : ""}  `);
      L.push(`**Strategic linkage:** ${p.strategic_linkage || "none identified"}  `);
      ...
      [["Result", m.result], ["Volume", m.volume], ["Unit cost", m.unit_cost]].forEach(([k, x]) => L.push(`| ${k} | ${x?.metric || "—"} | ${x?.source || ""} | ${x?.exists_today || ""} |`));
              ...
              <div style={{ marginBottom: 32 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 8, flexWrap: "wrap", gap: 6 }}>
                  <div style={{ fontSize: narrow ? 18 : 22 }}>{programs.length} programs · {money(total)} · {programs.reduce((a, p) => a + (Number(p.fte) || 0), 0).toFixed(1)} FTE
                    <span style={{ fontFamily: sans, fontSize: 13, color: C.grey, marginLeft: 10 }}>{result.orientation}-facing</span></div>
                ...
                return (
                  <section key={key} id={`prog-${encodeURIComponent(p.name)}`} className="program-card"
                    ...
                    <div style={{ padding: `18px ${pad}px 14px`, display: "grid", gridTemplateColumns: narrow ? "1fr" : "1fr auto", gap: narrow ? 12 : 20, alignItems: "start" }}>
                      <div>
                        ...
                        <div style={{ marginTop: 10, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                          ...
                          {p.strategic_linkage && <span style={{ fontFamily: sans, fontSize: 11, color: /none identified/i.test(p.strategic_linkage) ? C.grey : C.maroon, border: `1px solid ${/none identified/i.test(p.strategic_linkage) ? C.line : C.maroon}`, padding: "2px 8px" }}>{p.strategic_linkage}</span>}
                        </div>
                      </div>
                    ...
                    <div style={{ borderTop: `1px solid ${C.line}`, display: "grid", gridTemplateColumns: narrow ? "1fr" : "1fr 1fr 1fr" }}>
                      <MeasureCell kind="Result" sub="did the condition change" m={m.result} last={narrow} />
