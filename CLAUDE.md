...
- The nine-field program record shape (name, statement, beneficiary, fte, measures.{result,volume,unit_cost}, mandate, strategic_linkage, twenty_percent, is_admin) mirrors the Program Standard document's section 4 field-for-field. If the Standard changes, update the prompt schema and every place that reads `p.<field>` (the card renderer, the CSV/Markdown/HTML exporters, and the budget-office `checks` array) together — they're currently kept in sync by hand, not by a shared type.
